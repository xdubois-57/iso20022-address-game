 ...
 * Logging in regenerates the session and deletes the old one, so a poll still
 * in flight — the countdown ticks every second — is handed a brand new session
 * and a brand new CSRF token, and the token this function returned is dead
 * before it is used. See displayModeToken() in ../support/display-mode.js.
 */
