...
# The decisions worth testing live in a library so a test can call exactly the
# code a release runs — see the header of scripts/release-lib.sh for why that
# matters here in particular.
# shellcheck source=scripts/release-lib.sh
. "$(cd "$(dirname "$0")" && pwd)/scripts/release-lib.sh"
...
NEW_VERSION="$(release_next_version "$BUMP" "$LATEST_TAG")"
...
open_and_merge_release_pr() {
    ...
    release_write_version_file "$SCRIPT_DIR/config/version.php" "$NEW_VERSION" "$COMMIT_SHORT"
...
release_build_artifact "$ARTIFACT"

echo "Artifact built: $ARTIFACT ($(du -h "$ARTIFACT" | cut -f1))"

# ── What is asserted about the artifact, and why it is asserted here ────────
#
# Before anything is attached to a Release, so a surprise costs a re-run rather
# than a published bad artifact. The checks themselves live in the library and
# are covered by tests/release; what belongs here is the consequence, which is
# release-specific: the tag is already pushed by this point.
#
# The secrets check is first because it is the one with a published precedent.
# config/deploy.conf — the production FTP host, user and password — went out in
# the zip attached to v0.3.1, v0.3.2 and v0.3.3, and no gate noticed, because no
# gate looks inside the artifact. Releases are immutable, so those assets could
# not be withdrawn afterwards; rotating the credential was the only remedy left.
# That is the whole argument for failing here rather than apologising later.
if ! release_assert_no_local_secrets "$ARTIFACT"; then
    echo "" >&2
...
if ! release_assert_no_stray_copies "$ARTIFACT"; then
...
if ! release_assert_has_autoload "$ARTIFACT"; then
